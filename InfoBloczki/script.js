...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (hs != null) {
    ...
    document.getElementById("highestscore").innerHTML =
      `Najwyższy wynik ma ${hs["name"]} z wynikiem ${hs["score"]}`;
  } else {
  ...
  function mergeBlocks() {
    currentBlock.shape.forEach((row, y) => {
      row.forEach((value, x) => {
        if (value) {
          board[y + currentBlock.y][x + currentBlock.x] =
            colors[currentBlock.type];
        }
      });
    });
  }
  ...
  function moveDown() {
    if (
      ...
      if (
        collisionDetection(currentBlock.shape, currentBlock.x, currentBlock.y)
      ) {
        running = false;
        if (hs != null) {
          if (score > hs.score) {
  ...
  function Correct() {
    ...
    beg_questions += 1;
    if (beg_questions > 19) {
      document.getElementById("questionbox").innerHTML = "Pytanie nr. " + (beg_questions + 1);
    } else {
